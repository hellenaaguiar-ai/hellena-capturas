...
_active_sessions: dict[str, tuple[RecordingSession, datetime, RecordingIndicator, str]] = {}
    ...
    try:
        with _pipeline_lock:
            ...
            try:
                ...
                _notify("Erro na captura", f"{display_label}: {exc}")
                return

            write_errors_note(config.error_note_path, state.errors())

            if result == "done":
                _notify("Captura concluída", f"{display_label} processado e salvo no Obsidian.")
            elif result == "skipped-duplicate":
                logging.info("Gravacao de %s ja processada (duplicada)", mode.key)
            else:
                _notify("Falha na captura", f"{display_label} falhou — veja _Erros de captura.md no vault.")
    finally:
...
def _safe_toggle(config: Config, mode: CaptureMode, display_label: Optional[str] = None) -> None:
    ...
    thread interna da biblioteca `keyboard`. display_label sobrescreve
    mode.label so nas notificacoes/indicador (ex: o atalho de Reflexao usa
    o mesmo mode.key "idea" por baixo, mas deve aparecer como "Reflexão",
    nao "Ideia")."""
    label = display_label or mode.label
    logging.info("Atalho do modo '%s' acionado.", label)
    try:
        _toggle(config, mode, label)
    except Exception:
...
def _stop_mode(config: Config, mode: CaptureMode) -> None:
    ...
    session, recorded_at, indicator, display_label = _active_sessions.pop(mode.key)
    ...
    try:
        ...
        _notify("Erro na gravação", f"{display_label}: {exc}")
        indicator.hide()
        return

    _notify("Processando...", f"{display_label} — transcrevendo e estruturando.")
    thread = threading.Thread(
        target=_process_in_background,
        args=(config, mode, mic_path, system_path, recorded_at, indicator, display_label),
        daemon=True,
    )
    thread.start()
...
def _toggle(config: Config, mode: CaptureMode, display_label: Optional[str] = None) -> None:
    label = display_label or mode.label
    ...
    try:
        ...
        _notify("Erro ao gravar", f"{label}: {exc}")
        return

    indicator = RecordingIndicator(label)
    indicator.start()
    _active_sessions[mode.key] = (session, datetime.now().astimezone(), indicator, label)
    _update_tray_state()
    _notify(
        "Gravando...",
        f"{label} — aperte a mesma tecla de novo (ou o atalho de Parar) para parar.",
    )
